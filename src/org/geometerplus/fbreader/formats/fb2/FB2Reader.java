...
public final class FB2Reader extends ZLXMLReaderAdapter {
	...
	void readBook() {
		...
		try {
			final ZLFile file = ZLFile.createFileByPath(myBookReader.m_bookModel.Book.m_filePath);
	...
	public boolean startElementHandler(String tagName, ZLStringMap attributes) {
		...
		switch (tag) {
			...
			case FB2Tag.BODY:
				++myBodyCounter;
				myParagraphsBeforeBodyNumber = myBookReader.m_bookModel.myParagraphsNumber;
			...
			case FB2Tag.IMAGE:
			{
				...
				if ((imgRef != null) && (imgRef.length() != 0) && (imgRef.charAt(0) == '#')) {
					...
					final boolean isCoverImage =
						...
						myBookReader.m_bookModel.myParagraphsNumber;
