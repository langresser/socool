...
public class BookmarkManageActivity extends TabActivity implements MenuItem.OnMenuItemClickListener {
...
//		Collections.sort(AllBooksBookmarks, new Bookmark.ByTimeComparator());
		...
		if (FBReaderApp.Instance().Model != null) {
			...
			new BookmarkTitleAdapter(myThisBookView, myThisBookBookmarks, true);
		} else {
			findViewById(R.id.this_book).setVisibility(View.GONE);
		}

		myAllBooksView = createTab("allBooks", R.id.all_books);
		new BookmarkTitleAdapter(myAllBooksView, AllBooksBookmarks, false);
	...
	void showSearchResultsTab(LinkedList<Bookmark> results) {
		if (mySearchResultsView == null) {
			...
			new BookmarkTitleAdapter(mySearchResultsView, mySearchResults, false);
		} else {
	...
	public boolean onContextItemSelected(MenuItem item) {
		...
		final Bookmark bookmark = ((BookmarkTitleAdapter)view.getAdapter()).getItem(position);
		switch (item.getItemId()) {
			case OPEN_ITEM_ID:
	...
	private final class BookmarkTitleAdapter extends BaseAdapter implements AdapterView.OnItemClickListener, View.OnCreateContextMenuListener {
		private final List<Bookmark> myBookmarks;
		private final boolean myCurrentBook;

		BookmarkTitleAdapter(ListView listView, List<Bookmark> bookmarks, boolean currentBook) {
